 ...
 * Icons come OUT of folder one by one -> spread triangular -> folder SHATTERS like glass -> glue to wall
 * Responsive for mobile, laptop, and landscape
 */
...
export const FloatingButtons = () => {
  ...
  // Phase 0: Nothing
  // Phase 1: Folder visible
  // Phase 2: Icons come out one by one, spread triangle
  // Phase 3: Folder shatters like glass
  // Phase 4: Icons glue to wall
  ...
  // Auto show callback modal after 5 seconds
  useEffect(() => {
    if (!hasShownModal) {
  ...
  return (
    <>
      {/* Fixed container at bottom right for folder */}
      <div className="fixed right-4 bottom-4 sm:right-6 sm:bottom-6 z-[9999]">
        ...
        {/* ICONS - Come out of folder, spread triangle */}
        <AnimatePresence>
          {phase >= 2 && phase < 4 && (
            <>
              {/* WhatsApp - top position */}
              <motion.div
        ...
        {phase === 4 && (
          <>
            ...
            <motion.a
              ...
              initial={{ right: 20, bottom: 60, borderRadius: '50%' }}
              animate={{ right: 0, bottom: 170, borderRadius: '12px 0 0 12px' }}
              transition={{ duration: 0.5, type: 'spring', stiffness: 150, damping: 15, delay: 0 }}
            ...
            <motion.a
              href={callUrl}
              className="fixed w-11 h-11 sm:w-12 sm:h-12 bg-primary rounded-l-xl shadow-lg flex items-center justify-center cursor-pointer z-[9998]"
              initial={{ right: 55, bottom: 10, borderRadius: '50%' }}
              animate={{ right: 0, bottom: 115, borderRadius: '12px 0 0 12px' }}
            ...
            <motion.button
              ...
              className="fixed w-11 h-11 sm:w-12 sm:h-12 bg-primary rounded-l-xl shadow-lg flex items-center justify-center cursor-pointer z-[9998]"
              initial={{ right: 55, bottom: 60, borderRadius: '50%' }}
              animate={{ right: 0, bottom: 60, borderRadius: '12px 0 0 12px' }}
              transition={{ duration: 0.5, type: 'spring', stiffness: 150, damping: 15, delay: 0.2 }}
              whileHover={{ x: -5, scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
            >
      ...
      <AnimatePresence>
        {showCallbackModal && (
          <>
            ...
            <motion.div
              className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[90%] max-w-md bg-white rounded-2xl shadow-2xl z-[10001] overflow-hidden"
